...
mode_img, mode_hover_img = load_button("assets/images/BUTTONS/MODEPNG.png")

# Vertically aligned menu buttons layout
start_rect = start_img.get_rect(center=(WIDTH//2, 310))
mode_rect = mode_img.get_rect(center=(WIDTH//2, 415))
exit_rect = exit_img.get_rect(center=(WIDTH//2, 520))

# Sub-menu Mode Selection UI Assets
easy_img = pygame.transform.scale(pygame.image.load("assets/images/BUTTONS/EASYPNG.png").convert_alpha(), (140, 60))
medium_img = pygame.transform.scale(pygame.image.load("assets/images/BUTTONS/MEDIUMPNG.png").convert_alpha(), (140, 60))
hard_img = pygame.transform.scale(pygame.image.load("assets/images/BUTTONS/HARDPNG.png").convert_alpha(), (140, 60))
...
show_modes_menu = False

difficulty_settings = {
    0: {"gap": 210, "speed": 5, "label": "EASY"},
    1: {"gap": 180, "speed": 7, "label": "MEDIUM"},
    2: {"gap": 140, "speed": 10, "label": "HARD"}
}

#========MAP SELECTION SETUP============
current_map = 0 
map_labels = {0: "CYCLE ALL", 1: "CITY ONLY", 2: "FOREST ONLY", 3: "SNOW ONLY"}
map_names = ["CYCLE ALL", "CITY", "FOREST", "SNOW"]
...
def draw_menu():
    global easy_rect, medium_rect, hard_rect, map_choice_rects
    if sound_on and not pygame.mixer.music.get_busy():
        pygame.mixer.music.play(-1)

    screen.blit(bgs[0],(0,0))
    draw_text_outline(screen,"FLAP IT",title_font,WHITE,BLACK,(WIDTH//2,110))
    draw_text_outline(screen,f"HIGH SCORE: {high_score}",score_font,WHITE,BLACK,(WIDTH//2,165))
    draw_text_outline(screen,f"MODE: {difficulty_settings[current_difficulty]['label']}",instruction_font,YELLOW,BLACK,(WIDTH//2,205))
    draw_text_outline(screen,f"MAP: {map_labels[current_map]}",instruction_font,WHITE,BLACK,(WIDTH//2,230))
    ...
    screen.blit(mode_hover_img if mode_rect.collidepoint(mouse) else mode_img, mode_rect)
    ...
    if show_skins_menu:
    ...
    elif show_modes_menu:
        modes_surf = pygame.Surface((530, 390))
        modes_surf.set_alpha(245)
        modes_surf.fill((15, 15, 15))
        modes_menu_rect = modes_surf.get_rect(center=(WIDTH//2, HEIGHT//2))
        screen.blit(modes_surf, modes_menu_rect)

        draw_text_outline(screen, "DIFFICULTY", score_font, YELLOW, BLACK, (WIDTH//2 - 125, HEIGHT//2 - 140))
        draw_text_outline(screen, "SELECT MAP", score_font, YELLOW, BLACK, (WIDTH//2 + 125, HEIGHT//2 - 140))

        easy_rect = easy_img.get_rect(center=(WIDTH//2 - 125, HEIGHT//2 - 50))
        medium_rect = medium_img.get_rect(center=(WIDTH//2 - 125, HEIGHT//2 + 25))
        hard_rect = hard_img.get_rect(center=(WIDTH//2 - 125, HEIGHT//2 + 100))

        if current_difficulty == 0: pygame.draw.rect(screen, YELLOW, easy_rect.inflate(12, 12), 4)
        if current_difficulty == 1: pygame.draw.rect(screen, YELLOW, medium_rect.inflate(12, 12), 4)
        if current_difficulty == 2: pygame.draw.rect(screen, YELLOW, hard_rect.inflate(12, 12), 4)
        ...
        for i, name in enumerate(map_names):
            r = pygame.Rect(0, 0, 190, 42)
            r.center = (WIDTH//2 + 125, HEIGHT//2 - 60 + (i * 54))
            map_choice_rects.append(r)
            
            bg_color = (40, 40, 40) if i != current_map else (100, 100, 0)
            pygame.draw.rect(screen, bg_color, r)
            pygame.draw.rect(screen, YELLOW if i == current_map else WHITE, r, 2)
            
            draw_text_outline(screen, name, instruction_font, WHITE, BLACK, r.center)

        draw_text_outline(screen, "CLICK OUTSIDE / MODE TO CLOSE", instruction_font, GRAY, BLACK, (WIDTH//2, HEIGHT//2 + 165))
...
def run_game():
    global high_score, current_skin_index, current_difficulty, current_map

    bird_x, bird_y, bird_speed = 80, HEIGHT//2, 0
    gravity = 0.5

    pipe_width = 120
    pipe_gap = difficulty_settings[current_difficulty]["gap"]
    pipe_speed = difficulty_settings[current_difficulty]["speed"]
    ...
    # Dynamic transition tracking variable
    transition_alpha = 0
    ...
    while running:
        ...
        #======= SUNSET / DAY-NIGHT CYCLE LOGIC =======
        # Determine base environment pairs based on choices
        if current_map == 0:
            env_cycle = (score // 10) % 3  # Rotate City(0), Forest(1), Snow(2) every 10 points
            day_idx = env_cycle * 2
            night_idx = day_idx + 1
            # Sun sets dynamically on the second half of every world block
            score_in_env = score % 10
        elif current_map == 1:
            day_idx, night_idx = 0, 1
            score_in_env = score % 10
        elif current_map == 2:
            day_idx, night_idx = 2, 3
            score_in_env = score % 10
        elif current_map == 3:
            day_idx, night_idx = 4, 5
            score_in_env = score % 10

        # Calculate a linear alpha target based on current score progression (Simulating Sunset)
        if score_in_env < 4:
            target_alpha = 0     # Pure Day
        elif score_in_env >= 7:
            target_alpha = 255   # Pure Night
        else:
            # Scale alpha cleanly across intermediate scores (4, 5, 6)
            target_alpha = int(((score_in_env - 4) / 3) * 255)

        # Smooth interpolation to simulate gradual lighting drops frames
        if transition_alpha < target_alpha:
            transition_alpha = min(transition_alpha + 3, target_alpha)
        elif transition_alpha > target_alpha:
            transition_alpha = max(transition_alpha - 3, target_alpha)

        # Render background scene with transition blend layers
        screen.blit(bgs[day_idx], (0, 0))
        if transition_alpha > 0:
            night_bg_copy = bgs[night_idx].copy()
            night_bg_copy.set_alpha(transition_alpha)
            screen.blit(night_bg_copy, (0, 0))

        # Pipe configurations matching active lighting
        pipe_style_index = (night_idx if transition_alpha > 128 else day_idx) // 2
    ...
    for event in pygame.event.get():
        ...
        if game_state == MENU and event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if show_modes_menu:
                ...
                map_clicked = False
                for idx, r in enumerate(map_choice_rects):
                    if r.collidepoint(pos):
                        current_map = idx
                        map_clicked = True
                
                if mode_rect.collidepoint(pos) or (not easy_rect.collidepoint(pos) and not medium_rect.collidepoint(pos) and not hard_rect.collidepoint(pos) and not map_clicked):
                    show_modes_menu = False
                continue
            ...
            elif mode_rect.collidepoint(pos):
                show_modes_menu = True
                show_help = False
                show_skins_menu = False
